...
internal fun updateAppWidget(
    ...
    views.setOnClickPendingIntent(R.id.home_widget, homeIntent)
        ...
        if (response.isSuccessful) {
           ...
           Picasso.get().load("https://openweathermap.org/img/wn/${weather?.icon}@2x.png").get()
            withContext(Dispatchers.Main) {
                ...
                views.setTextViewText(R.id.appwidget_text, city)
                views.setTextViewText(R.id.pressure, pressure.toString())
                views.setTextViewText(R.id.humidity, "${humidity.toString()} %")
                views.setImageViewBitmap(R.id.icon_image, icon)
